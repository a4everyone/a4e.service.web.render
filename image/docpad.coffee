...
dynaLoad = (confPath) ->
    tempVar = require (confPath)
    subConfigs.push tempVar(path, __dirname)

dynaLoad modulePath for modulePath in webFolders.map( (fullPath) -> path.join(fullPath, 'docpad.coffee') ) when fs.existsSync(modulePath)
...
docpadConfig = {
    ...
    #configPaths: webFolders.map( (fullPath) -> path.join(fullPath, 'docpad.coffee') )
    # configPaths: [
    #     '/website/main/docpad.coffee'
    # ]
    #
    # configPaths: [  # default
    #     'docpad.js'
    ...
    reloadPaths: webFolders.map( (fullPath) -> path.join(fullPath, 'docpad.coffee') ).filter( (fullPath) -> fs.existsSync(modulePath) )
    #   [
    #     __dirname
    #     '/website/main/docpad.coffee'
    # ]  # default
    ...
    regenerateDelay: 500    # default

    # Out Path
    # Where should we put our generated website files?
    # If it is a relative path, it will have the resolved `rootPath` prepended to it
    outPath: websiteOut  # default

    # Src Path
    # Where can we find our source website files?
    # If it is a relative path, it will have the resolved `rootPath` prepended to it
    srcPath: path.join(websiteSrc, 'main')  # default
    ...
    documentsPaths: webFolders.map( (fullPath) -> path.join(fullPath, 'render') )

    # Files Paths
    # An array of paths which contents will be treated as files
    # If it is a relative path, it will have the resolved `srcPath` prepended to it
    filesPaths: webFolders.map( (fullPath) -> path.join(fullPath, 'raw') )

    # Layouts Paths
    # An array of paths which contents will be treated as layouts
    # If it is a relative path, it will have the resolved `srcPath` prepended to it
    layoutsPaths: webFolders.map( (fullPath) -> path.join(fullPath, 'layouts') )
    ...
    ignoreCustomPatterns: /typings\.json$|\/typings\/|^typings\//  # default false
    ...
    collections:
        sitemapPages: ->
            @getCollection("html").findAll({sitemap:true})
    ...
    plugins:  # example
        ...
        cleanurls:
            ...
            staticRedirectFile: false # still not working becuase is implemented in non merged branch
        partials:
            performanceFirst: false # off = The template data must be specified in the partial include parameters exclusivly
            partialPaths: webFolders.map( (fullPath) -> path.join(fullPath, 'partials') )
            # partialsPath: '/website/main/partials'
        sitemap:
            cachetime: 600000
            changefreq: 'weekly'
            priority: 0.5
            filePath: 'sitemap-www.xml'
...
# Merge all child configurations
extendr.extend(docpadConfig, subConfig) for subConfig in subConfigs
